...
# service under user@) can take control of seat0 via logind.
if [ "$(tty)" = "/dev/tty1" ] && [ -z "${WAYLAND_DISPLAY:-}" ]; then
  systemctl --user import-environment XDG_SESSION_ID XDG_SEAT XDG_VTNR
  # Start weston; it pulls in graphical-session.target itself (that target
  # refuses manual start, so we must not start it directly).
