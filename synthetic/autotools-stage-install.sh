#!/usr/bin/env bash
# Autotools build script for the SandboxInstall builder, staging variant: builds
# with --prefix=/usr but installs into a virgin /stage (make install
# DESTDIR=/stage) instead of the live overlay root. The AutotoolsStageRootfs
# lowering then re-roots /stage -> / via TreeMove. This keeps the layer additive
# even when the package's output overlaps files already in the build rootfs
# (e.g. a toolchain member rebuilt on a rootfs that already carries it).
set -euo pipefail
...
# Fixed staging root, matched by the AutotoolsStageRootfs lowering's
...
resolve_project_source_dir() {
  ...
  if [ ! -d "$source_dir" ]; then
    echo "autotools-stage-install build-script: source input is not a directory: ${source_dir}" >&2
    exit 1
  fi
  ...
  if [ -f "${cfg}/source_subdir" ]; then
    ...
    case "$source_subdir" in
      ""|/*|*"/../"*|../*|*"./"*|*"/.."|"..")
        echo "autotools-stage-install build-script: invalid source_subdir '${source_subdir}'" >&2
        exit 1
        ;;
    esac
  fi
  ...
  if [ ! -d "$project_source_dir" ]; then
    echo "autotools-stage-install build-script: project source dir is not a directory: ${project_source_dir}" >&2
    exit 1
  fi

  if [ ! -x "$project_source_dir/configure" ]; then
    echo "autotools-stage-install build-script: ./configure not found in ${project_source_dir}" >&2
    exit 1
  fi
...
step_install() {
  ...
  prepare_tmpdir "$build_dir"
  append_dir_files_to_array "${cfg}/make_args" make_args
  # Install into the virgin ${stage_root} (configured --prefix=/usr, so paths
  # baked into binaries still point at /usr). The writes are all fresh additions
...
case "$step" in
  ...
  *)
    echo "autotools-stage-install build-script: unsupported step '$step'" >&2
    exit 1
    ;;
esac
